...
CREATE TRIGGER IF NOT EXISTS blog_posts_bd BEFORE DELETE ON blog_posts BEGIN
...
CREATE TRIGGER IF NOT EXISTS blog_posts_bu BEFORE UPDATE OF title, summary, body ON blog_posts BEGIN
    DELETE FROM blog_posts_fts WHERE rowid = old.post_id;
END;

-- 4. Sync on Update: Part 2 (Inserts the new FTS entry after the data changes)
CREATE TRIGGER IF NOT EXISTS blog_posts_au AFTER UPDATE OF title, summary, body ON blog_posts BEGIN
    INSERT INTO blog_posts_fts(rowid, title, summary, body)
    VALUES (new.post_id, new.title, new.summary, new.body);
END;
